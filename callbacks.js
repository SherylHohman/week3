// Before you get started, we're going to load this file into our index.html
  // file so we can see our console.logs show up in the browser like we did yesterday!
  // To do this, open up your index.html file in your favorite text editor.
  // Uncomment the script for today's callback exercises.
  // You can go ahead and comment out other scripts
  //  so that your console doesn't get unnecessarily cluttered.


// ********** CALLBACK EXERCISES **********

// Let's do a quick review of functions.
  // Remember that a function is just a block of code that we've created
  //  but not yet invoked.
  // An analogy for this is a recipe.
  // Writing a function is like writing a recipe.
  // Invoking a function is like actually baking that recipe.
  // Another way of thinking of this is that the function is a set
  //  of instructions, but we haven't yet carried out those instructions.
...
// 2. Let's create a function called breadMaker.
  ...
  // then console.logs 'mmm, fresh baked ' + argName + ' bread.'
...
// 4. Using a for loop,
  ...
  // At this point, you might be kind of bored.
  // Because this is exactly what we've been doing all along.
...
// 5. First, within each iteration,
  ...
  //  funcInvoker should be passed the function that we want it to invoke,
  //  as well as the argument we want passed into that function.
  ...
  // Right now it only works for this particular array
  //  (ingredientsArray) and this particular function (breadMaker).
  // If we wanted to invoke this on a different array or use a different function,
  //  we'd have to write the entire loop all over again.

// 6. Write a function called invokeOnEach
  ...
  // Let's explore another way of using callback functions:
  //  to call a function after some period of time has passed
  //  (say, after you've made an API request to get a video from YouTube,
...
// 7. Try creating various functions and
  ...
  // Try modifying a variable, and console.logging it before and after we
  ...
  // We can create a function and then store it into a variable, as we've done
  //  with var call5SecondsLater = function() {};
  // Or, we can create an anonymous function and not store it into a variable:
...
// This is a very common pattern for functional programming.
  // If we're creating a function that we'll only use
  //  for that particular higher-order invocation,
  //  it can be clearer to just write it as an anonymous function
  //  inside the higher-order function invocation.
  // This saves us from having to search elsewhere in our file
  //  to find where we defined that function and figure out what it does.
  //  It's a stylistic preference, but you should know each pattern
  //  - (saving a function into a variable and
...
// Remember back to that point that
  // a function is just a block of code we've created but not invoked yet?
  // Obviously, we can invoke it ourselves manually by
  //  putting () next to the function name.
  //  Or, we can let the machine invoke it for us, as we did for setTimeout.

// Extra Credit:
  // Let's put this together with the closures that we learned yesterday!
  // Create a function called onlyOnce that
  //  takes in a function as a callback and
  //  returns a function that only lets that callback be invoked once.

  // This kind of thing is useful in a variety of places:
  //  - charging credit cards for an order
  //    (we only want to charge the customer once per order), s
  //  - signing up a new user, or even something like
  //  - using a gift card or using a ticket.
...
// Extra Extra Credit:
  // Now let's create a better version of onlyOnce
  //  that lets us pass in arguments to the callback function!
    // Example:
    /*
    ...
    */
  // HINT: you'll need to use the arguments keyword and apply.
  //  Feel free to google for these if you want to explore further!
